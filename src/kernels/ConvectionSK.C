...
InputParameters validParams<ConvectionSK>()
{
  ...
  params.addRequiredCoupledVar("grad_velocity", "Field variable for gradient of convective term");
/*
...
Real
ConvectionSK::computeQpResidual()
{
  return _test[_i][_qp] * ( _grad_var[_qp] * _grad_u[_qp] );
}

Real
ConvectionSK::computeQpJacobian()
{
  return _test[_i][_qp] * ( _grad_var[_qp] * _grad_phi[_j][_qp] );
}

Real
ConvectionSK::computeQpOffDiagJacobian(unsigned int jvar)
{
